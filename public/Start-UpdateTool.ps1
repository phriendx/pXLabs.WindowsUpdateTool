function Start-UpdateTool {
	...
	$SettingsRegPath = "HKEY_Current_User\Software\pXLabs\MS Update Tool"
	$LogFile = "$($env:ProgramData)\pXLabs-MSUpdateTool\pXLabs_MSUpdateTool.log"
	$Script:SectionBreak = "----------------------------------------------"
	...
	IF (-not (Test-Path "$($env:ProgramData)\pXLabs-MSUpdateTool")) {
		New-Item -Path "$($env:ProgramData)\pXLabs-MSUpdateTool" -Force -ItemType Directory | out-null
	}
	...
	$controls["GetUpdates"].Add_Click({
			...
			$paramAddJobTracker = @{
				...
				ArgumentList = $computer, $SessionType, $cred, $CMClientPresent
			...
			if ($null -ne $selectedItems) {
				foreach ($item in $selectedItems) {
					...
					$paramAddJobTracker = @{
						...
						ArgumentList = $computer, $SessionType, $Title, $kbOrId, $cred, $CMClientPresent
	...
	$ThemePaths = @{ }
	$ThemeFiles = Get-ChildItem (Join-Path -Path $PSScriptRoot '..\ui\themes') -Filter *.xaml
	...
	$themeToLoad = $themeToLoad.ToLowerInvariant()
	...
	$SessionLoad = {
		...
		$window.Title = "pXLabs MS Update Tool - $ComputerName ($SessionType | $UpdateMethod)"
